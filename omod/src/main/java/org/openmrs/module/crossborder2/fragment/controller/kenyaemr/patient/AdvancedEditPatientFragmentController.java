 ...
 */
public class AdvancedEditPatientFragmentController {
	...
	final String clientRegistryClientVerificationApi = (administrationService
	        .getGlobalProperty(CommonMetadata.GP_CLIENT_VERIFICATION_GET_END_POINT));
	
	final String clientRegistryApiToken = (administrationService
	        .getGlobalProperty(CommonMetadata.GP_CLIENT_VERIFICATION_API_TOKEN));
	...
	public String getFacilityName(@RequestParam("facilityCode") String facilityCode, @SpringBean KenyaUiUtils kenyaUi,
	        UiUtils ui) {
		String ret = "";
	...
	public void controller(@FragmentParam(value = "patient", required = false) Patient patient,
		...
		model.addAttribute("enableClientNumberField",
		    (StringUtils.isBlank(clientNumberFieldEnabled) || clientNumberFieldEnabled.equalsIgnoreCase("false")) ? false
		            : true);
		...
		List<String> nextOfKinRelationshipOptions = Arrays.asList(new String("Partner"), new String("Spouse"), Dictionary
		        .getConcept(Dictionary.FATHER).getName().getName(), Dictionary.getConcept(Dictionary.MOTHER).getName()
		        .getName(), Dictionary.getConcept(Dictionary.GRANDMOTHER).getName().getName(),
		    Dictionary.getConcept(Dictionary.GRANDFATHER).getName().getName(), Dictionary.getConcept(Dictionary.SIBLING)
		            .getName().getName(), Dictionary.getConcept(Dictionary.CHILD).getName().getName(), Dictionary
		            .getConcept(Dictionary.AUNT).getName().getName(), Dictionary.getConcept(Dictionary.UNCLE).getName()
		            .getName(), Dictionary.getConcept(Dictionary.GUARDIAN).getName().getName(),
		    Dictionary.getConcept(Dictionary.FRIEND).getName().getName(), Dictionary.getConcept(Dictionary.CO_WORKER)
		            .getName().getName());
		...
		List<String> cadreOptions = Arrays.asList(new String("Troop"), new String("Civilian"));
		model.addAttribute("cadreOptions", cadreOptions);
		
		/*Create list of rank answer Options  */
		List<String> rankOptions = Arrays.asList(new String("General(Gen)"), new String("Lieutenant General (Lt Gen)"),
		    new String("Major General (Maj Gen)"), new String("Brigadier (Brig)"), new String("Colonel (Col)"), new String(
		            "Lieutenant Colonel (Lt Col)"), new String("Major (Maj)"), new String("Captain (Capt)"), new String(
		            "Lieutenant (Lt)"), new String("2nd Lieutenant (2lt)"), new String("Warrant officer 1 (WO1)"),
		    new String("Warrant officer 2 (WO2)"), new String("Senior Sergeant (Ssgt)"), new String("Sergeant (Sgt)"),
		    new String("Corporal (Cpl)"), new String("Private (Spte)"));
	...
	public class EditPatientForm extends AbstractWebForm {
		...
		public void validate(Object target, Errors errors) {
			...
			if (isKDoD.equals("true")) {
				validateIdentifierField(errors, "kDoDServiceNumber",
			...
			validateIdentifierField(errors, "birthCertificateNumber",
			...
			validateIdentifierField(errors, "nationalUniquePatientNumber",
		...
		public Patient save() {
			...
			if (anyChanges(toSave.getPersonAddress(), personAddress, "address1", "address2", "address5", "address6",
			    "countyDistrict", "address3", "cityVillage", "stateProvince", "country", "postalCode", "address4")) {
			...
			PatientIdentifierType openmrsIdType = MetadataUtils.existing(PatientIdentifierType.class,
			...
			handleOncePerPatientObs(ret, obsToSave, obsToVoid, Dictionary.getConcept(Dictionary.CIVIL_STATUS),
			    savedMaritalStatus, maritalStatus);
			handleOncePerPatientObs(ret, obsToSave, obsToVoid, Dictionary.getConcept(Dictionary.OCCUPATION),
			    savedOccupation, occupation);
			handleOncePerPatientObs(ret, obsToSave, obsToVoid, Dictionary.getConcept(Dictionary.EDUCATION), savedEducation,
			    education);
			handleOncePerPatientObs(ret, obsToSave, obsToVoid, Dictionary.getConcept(Dictionary.IN_SCHOOL), savedInSchool,
			    inSchool);
			handleOncePerPatientObs(ret, obsToSave, obsToVoid, Dictionary.getConcept(Dictionary.ORPHAN), savedOrphan, orphan);
			handleOncePerPatientObs(ret, obsToSave, obsToVoid, Dictionary.getConcept(Dictionary.COUNTRY), savedCountry,
			    country);
